...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  <div class="container">
  <a class="navbar-brand" href="#"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-gear" viewBox="0 0 16 16">
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <button type="button" class="btn-close btn-close-white" data-dismiss="modal" aria-label="Close" @click="toggleAddTaskModal"></button>
          </div>
          <div class="modal-body">
            <form>
              ...
              <div class="mb-3">
                ...
                <input type="integer" class="form-control" id="addPomodoro" v-model="addTaskForm.pomodoro" placeholder="3" min="1" max="10" required>
              </div>
              <div class="btn-group" role="group">
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <button type="button" class="btn-close btn-close-white" data-dismiss="modal" aria-label="Close" @click="toggleEditTaskModal"></button>
          </div>
          <div class="modal-body">
            <form>
              <div class="mb-3">
  ...
  <script>
    ...
    export default {
      ...
      methods: {
        handleAction() {
          if (this.action === 'Play' && this.selectedTask != null && this.selectedTask.status != 'Completed' && this.mode != 'Break') {
            this.startTimer();
        ...
        tick() {
          if (this.seconds > 0) {
            ...
            if (this.mode === 'Focus') {
              const status = this.selectedTask.pomodoro === 1 ? 'Completed' : this.selectedTask.status;
              const payload = {
        ...
        resetTimer() {
          if (this.minutes == 0) { // toggle between Focus/Break
            this.mode = this.mode === 'Focus' ? this.mode = 'Break': this.mode = 'Focus';
          }
          this.minutes = this.mode === 'Focus' ? this.pomodotime: this.breaktime;
          this.strokecolor = this.mode === 'Focus' ? '#51B170': '#6085C5';
          this.seconds = 0;
        },
        updateDasharray() { // handle trickle down effect
          const totalDuration = this.mode === 'Focus' ? this.pomodotime * 60: this.breaktime * 60;
        ...
        handleAddSubmit() {
          if (this.addTaskForm.name.trim() != '' && this.addTaskForm.pomodoro.trim() != '') {
